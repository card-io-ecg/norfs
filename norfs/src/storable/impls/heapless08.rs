...
impl<T, const N: usize> Loadable for heapless08::Vec<T, N>
where
    T: Loadable,
{
    async fn load<R: Read>(reader: &mut R) -> Result<Self, LoadError<R::Error>> {
        ...
        let mut vec = heapless08::Vec::new();
...
impl<T, const N: usize> Storable for heapless08::Vec<T, N>
where
    T: Storable,
{
...
impl<const N: usize> Loadable for heapless08::String<N> {
    async fn load<R: Read>(reader: &mut R) -> Result<Self, LoadError<R::Error>> {
        let count = usize::load(reader).await?;

        let mut string = heapless08::String::new();
...
impl<const N: usize> Storable for heapless08::String<N> {
    async fn store<W: Write>(&self, writer: &mut W) -> Result<(), W::Error> {
        self.len().store(writer).await?;
